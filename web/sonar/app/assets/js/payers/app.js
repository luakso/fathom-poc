...
import { $, $$ } from "../dom.js";
import { fmtUSDC, fmtCount, fmtInt, pct } from "../format.js";
...
function rOverview() {
  ...
  $("#ov-stats").innerHTML = `
    ...
    <div class="bignum c-cm glow">${pct(c.top10_volume, c.total_volume)}<small>TOP-10 SHARE OF USDC</small></div>`;
...
function rLeader() {
  ...
  const c = win().concentration;
  $("#skew").innerHTML = `<b>spend skew:</b> the top 10 payers drive <span class="c-cm">${pct(c.top10_volume, c.total_volume)}</span> of USDC and <span class="c-cm">${pct(c.top10_txns, c.total_txns)}</span> of payments across ${fmtInt(c.total_entities)} wallets.`;
...
function rShell() {
  ...
  const lines = [
    ...
    `<div class="out">distinct payers ${fmtInt(c.total_entities)} · ${fmtCount(c.total_txns)} payments · ${fmtUSDC(c.total_volume)}</div>`,
...
function pinForEntity(r) {
  return {
    ...
    value: fmtUSDC(r.volume_usdc) + " spent",
...
const PIN = {
  overview() { const c = win().concentration; return { title: "PAYERS · " + state.win.toUpperCase(), value: fmtUSDC(c.total_volume), context: `${fmtInt(c.total_entities)} distinct payers · ${fmtCount(c.total_txns)} payments · top-10 drive ${pct(c.top10_volume, c.total_volume)} of USDC`, denom: "x402 payers on Base · " + WIN_LABEL[state.win] }; },
  ...
  conc() { const c = win().concentration; return { title: "CONCENTRATION · " + state.win.toUpperCase(), value: pct(c.top10_volume, c.total_volume) + " to top 10", context: `top-10 payers drive ${pct(c.top10_volume, c.total_volume)} of USDC and ${pct(c.top10_txns, c.total_txns)} of payments; top-100 drive ${pct(c.top100_volume, c.total_volume)} of USDC`, denom: "concentration{} · cross-checked vs economy.json · " + WIN_LABEL[state.win] }; },
