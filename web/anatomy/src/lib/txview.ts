import { weiToEth, gwei, groupDigits, relativeAge } from './format'

export type Row = { k: string; v: string; note?: string }
export type TxView = {
  headline: { paid: string; fee: string; method: string; methodKind: string; age: string; events: string }
  sections: { title: string; rows: Row[] }[]
  input: { method: string; methodId: string; decodable: boolean; params: Row[] | null; rawCalldata: string }
  basescanUrl: string
}

const dash = (s: string | undefined) => (s == null || s === '' ? '—' : s)
const eth = (wei: string | undefined) => (wei == null || wei === '' ? '—' : `${weiToEth(wei)} Ξ`)
const allDash = (parts: string[]) => parts.every((p) => p === '—')

function utilization(limit: string | undefined, used: string | undefined): string {
...
export function buildTxView(f: Record<string, string>, nowMs: number): TxView {
